...
本项目使用 Unreal Engine 5 与 C++/蓝图混合开发，实现一个以格子为基础的俯视角塔防原型。玩家通过 WASD 平移视野、鼠标滚轮缩放镜头，从手牌选择防御塔并拖拽到合法格子；敌人按照关卡中的 Spline 路径移动，防御塔依据攻击范围和目标类型自动攻击，玩家通过费用部署，通过作战经验升级手牌。
